...
| 情境 | 組織名稱 | 地址欄位 | 顯示結果 |
|------|----------|----------|----------|
| 有組織 + 有地圖連結 | 🗺️ 數位發展部 (可點擊) | 隱藏或合併 | 單一整合欄位 |
| 有組織 + 自訂標籤不同 | 🏢 數位發展部 | 📍 台北辦公室 | 兩個獨立欄位 |
| 有組織 + 標籤相同 | 🗺️ 數位發展部 (可點擊) | 隱藏 | 避免重複顯示 |
| 無組織 + 有地圖 | 隱藏 | 📍 自訂標籤 | 標準地址顯示 |

#### 實作範圍
1. **雙語版本優先**:
   - 修改 `renderPersonalCard` 函式中的組織名稱顯示邏輯
   - 當有 Google Maps 連結時，組織名稱變成可點擊連結
   - 添加地圖圖示和適當的樣式

2. **單語版本同步**:
   - 修改 `index-personal.html` 和 `index-personal-en.html`
   - 保持與雙語版本一致的顯示邏輯
...
```javascript
// 組織名稱地圖連結化邏輯
if (data.organization && data.location && data.location.mapId) {
    const orgText = getLocalizedText(data.organization, lang);
    const mapUrl = `https://maps.app.goo.gl/${data.location.mapId}`;
    ...
    orgLink.textContent = `🗺️ ${orgText}`;
    orgLink.className = 'org-map-link';
    
    // 檢查是否與地址標籤重複
    const shouldHideAddressItem = checkDuplicateContent(orgText, addressLabel);
}
```
...
### 2025-10-23: 組織名稱地圖連結整合完成 ✅
- ✅ **需求分析**: 避免組織名稱與地址標籤重複顯示
- ✅ **功能設計**: 組織名稱地圖連結化，雙重功能整合
- ✅ **顯示邏輯**: 智慧去重機制，相同內容時優先顯示組織連結
- ✅ **雙語版本實作**: index-bilingual-personal.html 組織名稱地圖連結與智慧去重
- ✅ **單語版本實作**: index-personal.html / index-personal-en.html 同步實作
- ✅ **樣式整合**: CSS hover 效果與視覺一致性
- ✅ **UX 提升**: 功能整合、UI 簡潔、直覺操作
...
### v2.1.9 組織地圖連結整合測試
1. **組織地圖連結**：有組織名稱 + Google Maps 連結時，組織名稱變成可點擊地圖連結
2. **智慧去重顯示**：組織名稱與地址標籤相同時，只顯示組織地圖連結
3. **獨立欄位顯示**：組織名稱與地址標籤不同時，顯示兩個獨立欄位
4. **圖示整合**：組織地圖連結顯示地圖圖示（🗺️）
5. **雙語支援**：組織名稱地圖連結支援中英文切換
6. **樣式一致性**：地圖連結樣式與其他連結保持一致
...
2. **單語版本同步**：
   ...
   - 保持顯示邏輯一致性
3. **樣式設計**：
   - 新增 `.org-map-link` 樣式類別
   - 地圖圖示整合與 hover 效果
4. **測試驗證**：針對六項組織地圖連結測試情境進行驗證
